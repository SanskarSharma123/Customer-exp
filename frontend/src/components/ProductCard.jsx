...
const ProductCard = ({ product }) => {
  ...
  const toggleCompare = () => {
    ...
    let compareCategory = localStorage.getItem('compareCategory');
    let compareSubcategory = localStorage.getItem('compareSubcategory');

    if (isSelectedForCompare) {
      // Remove from selection
      updatedSelections = currentSelections.filter(id => id !== product.product_id);
      // If this was the last selected, clear category and subcategory
      if (updatedSelections.length === 0) {
        localStorage.removeItem('compareCategory');
        localStorage.removeItem('compareSubcategory');
      }
      localStorage.setItem('compareProducts', JSON.stringify(updatedSelections));
      setIsSelectedForCompare(false);
    ...
    if (currentSelections.length > 0) {
      compareCategory = localStorage.getItem('compareCategory');
      if (compareCategory && compareCategory !== String(product.category_id)) {
    ...
    // Set category and subcategory if this is the first selection
    if (updatedSelections.length === 1) {
      localStorage.setItem('compareCategory', String(product.category_id));
      localStorage.setItem('compareSubcategory', String(product.subcategory_id));
    }
    localStorage.setItem('compareProducts', JSON.stringify(updatedSelections));
    setIsSelectedForCompare(true);
    window.dispatchEvent(new Event('compareProductsChanged'));
  };

  const handleConfirm = () => {
    // User confirmed to compare across subcategories
    const currentSelections = JSON.parse(localStorage.getItem('compareProducts') || '[]');
    let updatedSelections = [...currentSelections, product.product_id];
    localStorage.setItem('compareProducts', JSON.stringify(updatedSelections));
    setIsSelectedForCompare(true);
  ...
  return (
    <>
      <ConfirmDialog
        ...
        message="These products are from different subcategories. Do you still want to compare them?"
