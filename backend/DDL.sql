...
ALTER TABLE reviews                                                 ADD COLUMN sentiment_score DECIMAL(4,2),
ADD COLUMN sentiment_label VARCHAR(20),
ADD COLUMN sentiment_confidence DECIMAL(4,3);
...
ALTER TABLE reviews                                                 ADD CONSTRAINT chk_sentiment_score 
CHECK (sentiment_score IS NULL OR (sentiment_score >= 0 AND sentiment_score <= 10));

ALTER TABLE reviews                                                 ADD CONSTRAINT chk_sentiment_label 
CHECK (sentiment_label IS NULL OR sentiment_label IN (
    'highly positive', 'positive', 'neutral', 'negative', 'highly negative'
));

ALTER TABLE reviews                                        
ADD CONSTRAINT chk_sentiment_confidence 
CHECK (sentiment_confidence IS NULL OR (sentiment_confidence >= 0 AND sentiment_confidence <= 1));

DELETE FROM products p
USING (
  SELECT MIN(product_id) AS keep_id, name, description, price, discount_price,
         category_id, image_url, stock_quantity, unit, is_featured
  FROM products
  GROUP BY name, description, price, discount_price,
           category_id, image_url, stock_quantity, unit, is_featured
) duplicates
WHERE p.name = duplicates.name
  AND p.description IS NOT DISTINCT FROM duplicates.description
  AND p.price = duplicates.price
  AND p.discount_price IS NOT DISTINCT FROM duplicates.discount_price
  AND p.category_id = duplicates.category_id
  AND p.image_url IS NOT DISTINCT FROM duplicates.image_url
  AND p.stock_quantity = duplicates.stock_quantity
  AND p.unit = duplicates.unit
  AND p.is_featured = duplicates.is_featured
  AND p.product_id <> duplicates.keep_id;
